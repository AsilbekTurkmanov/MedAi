        ...
        <main className="flex-1 p-6 md:p-8 overflow-y-auto max-w-7xl">
          <div className="mb-8">
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
              <FileText className="w-8 h-8 text-indigo-600 dark:text-indigo-400" /> Medical Audit Logs
            </h1>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                {logs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/50">
                    <td className="py-3 font-bold text-slate-900 dark:text-white">{log.userEmail}</td>
                    <td className="py-3 text-cyan-600 dark:text-cyan-400 font-medium">{log.action}</td>
                    <td className="py-3 text-slate-600 dark:text-slate-300">{log.entityType}</td>
